...
def _default_spawn(
    ...
    *,
    ...
    # Bundled kanban-worker skill: only add ``--skills kanban-worker`` when it
    # actually resolves for the home the worker runs under. The skill is absent
    # from many profile-scoped skills dirs, and preloading a missing skill is
    # fatal at CLI startup; omitting it is safe (the lifecycle contract still
    # ships via KANBAN_GUIDANCE). Migration 0.16->0.18.2: restauriert -- der
    # Merge hatte diese Naht + _kanban_worker_skill_available gedroppt.
