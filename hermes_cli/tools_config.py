...
def _get_platform_tools(
    ...
    *,
    ...
    # --- Jarvis no_mcp-Boden (ADR-0039 Invariante 1 / ADR-0031) --------------------
    # Der untrusted Voice-/Inbound-Pfad (Plattform ``api_server``) loest NIE Tools
    # ODER MCP-Server auf -- unabhaengig von platform_toolsets, Recovery-Block,
    # Plugin-Defaults oder global aktivierten MCP-Servern. NACHGEWIESEN (2026-07-11,
    # 0.18.2-Migration): der blosse Config-Pin ``api_server: []`` genuegt NICHT --
    # ohne den ``no_mcp``-Sentinel schleust der else-Zweig unten (enabled_mcp_servers)
    # alle global aktivierten MCP-Server ein. Darum diese zweite Verteidigungslinie:
    # fail-closed am Funktions-AUSGANG, churn-robust (klemmt das Endergebnis, nicht
    # eine interne Naht, die Upstream umbenennen koennte). enabled_toolsets traegt in
    # diesem Design SOWOHL Toolset-Keys ALS AUCH MCP-Server-Namen -> set() = beides null.
    # DEFERRED (Fork-Lock-Fix, Fabel-5/Pass 2): auf nativen gateway_hook/verify_hook
    # externalisieren, statt Kern-Edit.
