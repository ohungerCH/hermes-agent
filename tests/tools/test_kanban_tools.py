...
def test_trusted_surface_kanban_gate_is_session_scoped_without_ttl_leak(
    ...
    try:
        ...
        # Session ends: the trusted surface must vanish. ``clear_session_vars``
        # pins the ContextVars to "" ("explicitly cleared"); the assertions
        # below MUST run under that "" state, because that is exactly the
        # regression this test guards (a future ``clear_session_vars`` that
        # stopped clearing would leave the surface visible).
        ...
        # Restore the ``_UNSET`` sentinel so this test does not leave the
        # session ContextVars pinned to "" process-wide. ``clear_session_vars``
        # leaves them at "" ("explicitly cleared"), which suppresses the
        # ``os.environ`` fallback in ``get_session_env`` for every subsequent
        # test in this single-process file — that is what made
        # ``test_create_subscribes_gateway_session`` see platform="" and
