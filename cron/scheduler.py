...
def run_job(
    ...
    try:
        ...
        # The config.yaml fallback passes the RAW value (upstream 0.18.2): a YAML
        # boolean False means thinking disabled, so it must not be coerced through
        # str().strip() here (that would turn False into "False"). See
        # parse_reasoning_effort for the bool/None handling.
        from hermes_constants import parse_reasoning_effort
        _job_effort = job.get("reasoning_effort")
        if isinstance(_job_effort, str) and _job_effort.strip():
            reasoning_config = parse_reasoning_effort(_job_effort.strip())
        else:
